...
* *Kafka-based Data Generator*: Implemented in `AvroFlightDataGenerator.java`, generates random flight data and sends it to a Kafka topic using the Avro serializer and Schema Registry. Use Data Faker for Java library https://github.com/datafaker-net/datafaker to generate random data when needed.
